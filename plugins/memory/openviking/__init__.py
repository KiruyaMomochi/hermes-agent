...
class OpenVikingTimeouts:
    ...
    sync_join_timeout_seconds: float = _SYNC_JOIN_TIMEOUT
    session_commit_timeout_seconds: float = _SESSION_COMMIT_TIMEOUT
    shutdown_join_timeout_seconds: float = _SHUTDOWN_JOIN_TIMEOUT
    prefetch_generation_stale_seconds: float = _PREFETCH_GENERATION_STALE_TIMEOUT

    @classmethod
    def from_config(cls, data: Dict[str, Any]) -> "OpenVikingTimeouts":
        if not isinstance(data, dict):
            data = {}
        return cls(
            prefetch_join_timeout_seconds=_positive_finite_float(
                data.get("prefetch_join_timeout_seconds"), _PREFETCH_JOIN_TIMEOUT
            ),
            sync_join_timeout_seconds=_positive_finite_float(
                data.get("sync_join_timeout_seconds"), _SYNC_JOIN_TIMEOUT
            ),
            session_commit_timeout_seconds=_positive_finite_float(
                data.get("session_commit_timeout_seconds"), _SESSION_COMMIT_TIMEOUT
            ),
            shutdown_join_timeout_seconds=_positive_finite_float(
                data.get("shutdown_join_timeout_seconds"), _SHUTDOWN_JOIN_TIMEOUT
            ),
            prefetch_generation_stale_seconds=_positive_finite_float(
                data.get("prefetch_generation_stale_seconds"), _PREFETCH_GENERATION_STALE_TIMEOUT
            ),
        )
...
def _content_to_text(content: Any) -> str:
    """Convert Hermes multimodal/message content into OpenViking-safe text.

    Gateway turns may carry Anthropic/OpenAI-style content blocks (text,
    images, tool_use/tool_result, etc.). OpenViking's session and search APIs
    require plain strings, so normalize at the provider boundary instead of
    letting non-string payloads fail in the background worker.
    """
    if content is None:
        return ""
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._prefetch_started_at = 0.0
        self._timeouts = OpenVikingTimeouts()
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        """Return prefetched results, synchronously fetching on cache miss."""
        thread = self._prefetch_thread
        thread_alive_before = bool(thread and thread.is_alive())
        logger.debug(
            "OpenViking prefetch called: session_id=%s query_len=%d thread_alive=%s",
            session_id,
            len(_content_to_text(query)),
            thread_alive_before,
        )
        if thread_alive_before and thread is not None:
            stale_elapsed = time.monotonic() - self._prefetch_started_at if self._prefetch_started_at else 0.0
            if stale_elapsed >= self._timeouts.prefetch_generation_stale_seconds:
                logger.warning(
                    "OpenViking prefetch generation stale after %.1fs; falling back to synchronous fetch",
                    stale_elapsed,
                )
                with self._prefetch_lock:
                    self._prefetch_generation += 1
                    self._prefetch_result = ""
                result = self._fetch_prefetch_context(query, source="sync-stale-generation")
                logger.debug("OpenViking stale-generation fallback prefetch result_len=%d", len(result))
                if result:
                    return f"## OpenViking Context\n{result}"
            thread.join(timeout=self._timeouts.prefetch_join_timeout_seconds)
            if thread.is_alive():
                logger.warning(
                    "OpenViking prefetch thread still running after %.1fs join timeout; falling back to synchronous fetch",
                    self._timeouts.prefetch_join_timeout_seconds,
                )
                with self._prefetch_lock:
                    self._prefetch_generation += 1
                    self._prefetch_result = ""
                result = self._fetch_prefetch_context(query, source="sync-fallback")
                logger.debug("OpenViking sync fallback prefetch result_len=%d", len(result))
                if result:
                    return f"## OpenViking Context\n{result}"
        with self._prefetch_lock:
        ...
        if not result:
            logger.debug(
                "OpenViking prefetch cache empty after join: thread_alive_before=%s; fetching synchronously",
                thread_alive_before,
            )
            result = self._fetch_prefetch_context(query, source="sync-cache-miss")
            logger.debug("OpenViking sync cache-miss prefetch result_len=%d", len(result))
        else:
            logger.debug("OpenViking prefetch cache hit result_len=%d", len(result))
        if not result:
            return ""
        return f"## OpenViking Context\n{result}"
    ...
    def _fetch_prefetch_context(self, query: Any, *, source: str) -> str:
        query_text = self._normalize_prefetch_query(query)
        if not self._client or not query_text:
            logger.debug(
                "OpenViking %s prefetch skipped: has_client=%s query_len=%d",
                source,
                bool(self._client),
                len(query_text),
            )
        ...
        _ctx = _load_ctx()
        logger.debug(
            "OpenViking %s prefetch settings loaded: enabled=%s top_k=%s max_items=%s min_score=%s",
            source,
            _ctx.enabled,
            _ctx.top_k,
            _ctx.max_items,
        ...
        if not _ctx.enabled:
            logger.debug("OpenViking %s prefetch skipped: context control disabled", source)
            return ""
        client = _VikingClient(
            self._endpoint, self._api_key,
            account=self._account, user=self._user, agent=self._agent,
        )
        logger.debug("OpenViking %s prefetch POST /api/v1/search/find", source)
    ...
    def queue_prefetch(self, query: Any, *, session_id: str = "") -> None:
        """Fire a background search to pre-load relevant context."""
        query_text = self._normalize_prefetch_query(query)
        logger.debug(
            "OpenViking queue_prefetch entered: has_client=%s session_id=%s query_len=%d",
            bool(self._client),
            session_id,
            len(query_text),
        )
        if not self._client or not query_text:
            logger.debug("OpenViking queue_prefetch skipped: missing client or query")
            return
        with self._prefetch_lock:
            self._prefetch_generation += 1
        ...
        def _run():
            try:
                logger.debug("OpenViking prefetch thread started: generation=%d", generation)
                result = self._fetch_prefetch_context(query_text, source="background")
                if result:
                    with self._prefetch_lock:
                        if generation == self._prefetch_generation:
                            ...
                            logger.debug(
                                "OpenViking prefetch thread discarded stale result: generation=%d current=%d result_len=%d",
                                generation,
                                self._prefetch_generation,
                                len(result),
                            )
            except Exception as e:
                logger.warning("OpenViking prefetch failed: %s", e, exc_info=True)
    ...
    def sync_turn(
        ...
        *,
        ...
        if self._sync_thread and self._sync_thread.is_alive():
            self._sync_thread.join(timeout=self._timeouts.sync_join_timeout_seconds)
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        if self._sync_thread and self._sync_thread.is_alive():
            self._sync_thread.join(timeout=self._timeouts.session_commit_timeout_seconds)
    ...
    def shutdown(self) -> None:
        ...
        for t in (self._sync_thread, self._prefetch_thread):
            if t and t.is_alive():
                t.join(timeout=self._timeouts.shutdown_join_timeout_seconds)
