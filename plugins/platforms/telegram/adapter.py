...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _try_send_rich_draft(self, chat_id: str, draft_id: int, content: str, metadata: Optional[Dict[str, Any]]) -> bool:
        ...
        try:
            result = bool(await _await_with_thread_deadline(
    ...
    async def _send_text_locked(
        ...
        """Send under the per-chat lock, optionally fanning out delimiter-separated bubbles."""
        ...
        if split and getattr(self, "_split_replies_enabled", False):
            ...
            if len(parts) > 1:
                ...
                for index, (part, dash_count, part_offset) in enumerate(parts):
                    ...
                    result = await self._send_single_message(chat_id, part, part_reply_to, split_metadata)
                    self._trace_send(
                    ...
                    if not result.success:
                        ...
                        return result
                    raw_ids = result.raw_response.get("message_ids", []) if isinstance(result.raw_response, dict) else []
                    if raw_ids:
    ...
    async def send_reasoning(
        ...
        if not content or not content.strip():
            return SendResult(success=True, message_id=None)
        async with self._chat_send_lock(chat_id):
            return await self._send_text_locked(chat_id, content, None, metadata, split=False)
