...
def _is_markdown_v2(parse_mode) -> bool:
    """Accept both PTB enum values and string/mocked ParseMode constants."""
    return (
        parse_mode == "MarkdownV2"
        or str(parse_mode) == "MarkdownV2"
        or str(parse_mode).endswith(".MARKDOWN_V2")
        or "MARKDOWN_V2" in repr(parse_mode)
    )
...
class TestTelegramExecApproval:
    ...
    async def test_send_update_prompt_escapes_dynamic_prompt(self):
        ...
        assert result.success is True
        assert _is_markdown_v2(sent["parse_mode"])
...
class TestTelegramApprovalCallback:
    ...
    async def test_approval_callback_escapes_dynamic_user_name(self):
        ...
        assert _is_markdown_v2(edit_kwargs["parse_mode"])
