...
@pytest.mark.asyncio
async def test_delimiter_fanout_plans_once_and_sends_each_part_directly(telegram_adapter):
    """Fanout uses the single-part operation; it never recurses through send()."""
    telegram_adapter._send_single_message = AsyncMock(
        side_effect=[
            SendResult(success=True, message_id="401"),
            SendResult(success=True, message_id="402"),
        ]
    )

    result = await telegram_adapter.send(
        "12345", "First\n\n---\n\nSecond", reply_to="999", metadata={"thread_id": "42"},
    )

    assert result.success is True
    calls = telegram_adapter._send_single_message.await_args_list
    assert len(calls) == 2
    assert calls[0].args[2] == "999"
    assert calls[1].args[2] is None
    assert all("_telegram_split_part" not in (call.args[3] or {}) for call in calls)
